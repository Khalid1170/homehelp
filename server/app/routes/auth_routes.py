...
# ==========================================
@auth_bp.route("/register/client", methods=["POST", "OPTIONS"])
def register_client():
    # Explicitly catch browser preflight options
    if request.method == "OPTIONS":
        return jsonify({"status": "ok"}), 200
    ...
    # Lightweight validation for standard clients
    if not name or not email or not password:
        return jsonify({"error": "Missing mandatory registration fields (name, email, password)"}), 400
    ...
    try:
        # Create core user explicitly assigned as a client
        user = User(
            full_name=name,
            email=email,
            phone_number=phone,
        ...
        # Automatically build out the blank worker record shell structure
        worker = Worker(
            user_id=user.id,
            bio="",
            skills="",
        ...
        db.session.commit()
        return jsonify({"message": "Client account profile and structural database models successfully created."}), 201
...
# ==========================================
@auth_bp.route("/register/worker", methods=["POST", "OPTIONS"])
def register_worker():
    # Explicitly catch browser preflight options
    if request.method == "OPTIONS":
        return jsonify({"status": "ok"}), 200
    ...
    # Specific incoming worker profile data fields
    bio = data.get("bio", "")
    skills = data.get("skills", "")
    location_text = data.get("location_text", "")

    # Enforce stricter mandatory checks for service providers straight away
    if not name or not email or not password:
        return jsonify({"error": "Missing mandatory registration fields (name, email, password)"}), 400
        
    if not bio or not skills:
        return jsonify({"error": "Workers must provide profile details (bio and skills) to register."}), 400
    ...
    try:
        # Create core user explicitly assigned as a worker
        user = User(
            full_name=name,
            email=email,
            phone_number=phone,
        ...
        # Populate the active worker row instantly with their onboarding input
        worker = Worker(
            user_id=user.id,
            bio=bio,
            skills=skills,
        ...
        db.session.commit()
        return jsonify({"message": "Worker account profile and worker models successfully created."}), 201
...
def login():
    ...
    # Secondary lookup for worker profile id context (Now always guaranteed to exist!)
    worker_profile = Worker.query.filter_by(user_id=user.id).first()
